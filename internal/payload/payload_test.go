...
func TestEffortLevelDistinguishesAbsentFromEmpty(t *testing.T) {
	if _, ok := parseT(t, `{}`).EffortLevel(); ok {
		t.Error("EffortLevel() on {} reported present")
	}
	if _, ok := parseT(t, `{"effort":{}}`).EffortLevel(); ok {
		t.Error("EffortLevel() on an object with no level reported present")
	}
	if _, ok := parseT(t, `{"effort":{"level":null}}`).EffortLevel(); ok {
		t.Error("EffortLevel() on an explicit null reported present")
	}

	for _, want := range []string{"low", "medium", "high", "xhigh", "max", "glacial"} {
		got, ok := parseT(t, `{"effort":{"level":"`+want+`"}}`).EffortLevel()
		if !ok || got != want {
			t.Errorf("EffortLevel() = (%q, %v), want (%q, true)", got, ok, want)
		}
	}
}
