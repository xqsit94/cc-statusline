...
// Fixtures are synthetic .git trees in t.TempDir(). Nothing here shells out to
// git or depends on the repository the tests happen to run inside — a test that
// reads the real .git passes or fails based on which branch the developer is
// on, which is the definition of a flaky test.
...
func TestDiscoverBranch(t *testing.T) {
	cases := []struct {
		...
		{
			// An interrupted rebase detaches HEAD at the commit being replayed.
			// The SHA is meaningless to the user; head-name is the branch they
			// think they are on.
		...
		{
			// A bare repo has no working tree and no branch worth showing.
...
func TestDiscoverEnvironment(t *testing.T) {
	...
	t.Run("GIT_DIR skips the walk", func(t *testing.T) {
		...
		// startDir points at a different repo entirely; GIT_DIR must win.
...
func TestDiscoverAbsences(t *testing.T) {
	t.Run("no repository anywhere above", func(t *testing.T) {
		// t.TempDir() is under /tmp, which is not inside a git repository.
	...
	t.Run("a directory that no longer exists", func(t *testing.T) {
		// The case os.Getwd() cannot survive, and why the start directory
		// comes from the payload instead.
