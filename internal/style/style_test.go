...
// TestColorSurvivesAPipe and TestProfileIsHonoured are the two halves of
// PRD §6.5. The failure they guard is invisible in both directions: colour
// silently disappearing, and colour silently ignoring the resolved profile.
// Neither produces an error anywhere.

func TestColorSurvivesAPipe(t *testing.T) {
	// The pipe is the entire point. Claude Code captures stdout, so a test on
	// a TTY passes trivially and proves nothing — which is exactly how the
	// original bug survived three reviews.
	...
	out, err := cmd.Output() // captured, therefore a pipe
	if err != nil {
...
func TestProfileIsHonoured(t *testing.T) {
	// The regression this catches: termenv.WithProfile is silently ignored by
	// lipgloss, so every profile emitted truecolor. Asserting only "escapes
	// exist" would have passed throughout.
	cases := []struct {
		name string
		env  map[string]string
		want string // exact prefix, or "" for no escapes at all
	}{
...
func TestDetectIcons(t *testing.T) {
	cases := []struct {
		...
		// PRD §6.1: ASCII is the compatibility floor, so it wins. A user who
		// asked for it has a terminal that cannot show the alternative.
...
func TestPowerlineFollowsTheFont(t *testing.T) {
	cases := []struct {
		...
		// Honouring this would emit U+E0B0 into a terminal the user has just
		// told us cannot render Unicode.
...
func TestAmbiguousWidth(t *testing.T) {
	// `▓` `░` `◆` `⚠` are East Asian Ambiguous: under a CJK locale they render
	// double-width and every width calculation downstream doubles with them.
	cases := []struct {
		name string
		env  map[string]string
		want int
...
func TestGlyphOverrides(t *testing.T) {
	// PRD §6.2: an explicit value overrides the table for every icon set, but
	// selecting ASCII does not rewrite an explicit value. Presets ship "auto"
	// precisely so the icon set stays effective.
	c := config.Defaults()
